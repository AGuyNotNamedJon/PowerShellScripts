...
try {
    if (-not (Get-Command "netsh" -ErrorAction SilentlyContinue)) {
        Write-Log @LoggingSettings -Comment "The 'netsh' command is not available on this system. Aborting." -Level ERROR
        exit
    }
} catch {
    Write-Log @LoggingSettings -Comment "Error checking for netsh command: $_" -Level ERROR
    exit
}
...
foreach ($group in $conflictingGroups) {
    ...
    if ($activeParams.Count -gt 1) {
        ...
        Write-Log @LoggingSettings -Comment "Conflicting parameters detected: $($conflictingNames -join ', ')." -Level ERROR
        Write-Log @LoggingSettings -Comment "${group.Message}" -Level ERROR
        Write-Log @LoggingSettings -Comment "Try running the script with a single parameter from this group." -Level INFO
        exit
    }
}
...
if ($NetworkName) {
    if ($NetworkName -notmatch '^[a-zA-Z0-9 _-!@#]{1,32}$') {
		Write-Log @LoggingSettings -Comment "Invalid network name: '$NetworkName'." -Level ERROR
		Write-Log @LoggingSettings -Comment "Allowed characters: alphanumerics, spaces, underscores, hyphens, and special characters (!, @, #)." -Level ERROR
		Write-Log @LoggingSettings -Comment "Must be between 1 and 32 characters long." -Level ERROR
		exit
	}
}
...
function Create-BackupFolder {
	...
	if ($UseGUI -or $Backup) {
		if (-not (Test-Path -Path $BackupPath)) {
			Write-Log @LoggingSettings -Comment "Backup folder not found. Creating backup folder at $BackupPath..." -Level INFO
...
function ListSavedNetworks {
    ...
    Write-Log @LoggingSettings -Comment "Listing all saved Wi-Fi networks currently added to Windows..." -Level INFO
    ...
    if ($Networks) {
        $Networks | ForEach-Object { Write-Log @LoggingSettings -Comment $_ -Level INFO }
		if ($UseGUI) { Update-NetworkListView -Items $Networks }

    } else {
        Write-Log @LoggingSettings -Comment "No saved Wi-Fi networks found." -Level WARNING
        if ($UseGUI) { Update-NetworkListView -Items @("No networks found") }
    }
}
...
function ListBackedUpNetworks {
    ...
    Write-Log @LoggingSettings -Comment "Listing all Wi-Fi networks that have been backed up..." -Level INFO
    if (Test-Path $BackupPath) {
        ...
        if ($Networks) {
            ...
            Write-Log @LoggingSettings -Comment "No networks found in backup folder." -Level WARNING
            if ($UseGUI) { Update-NetworkListView -Items @("No backed-up networks found") }
        }
    } else {
        Write-Log @LoggingSettings -Comment "No backup folder found. Please perform a backup first." -Level WARNING
        if ($UseGUI) { Update-NetworkListView -Items @("Backup folder not found") }
    }
}
...
function Backup {
    ...
    Write-Log @LoggingSettings -Comment "Backing up saved Wi-Fi networks..." -Level INFO
    try {
        if ($NetworkName) {
            Write-Log @LoggingSettings -Comment "Backing up Wi-Fi network: $NetworkName..." -Level INFO
            ...
            if ($result -notmatch "successfully exported") {
                Write-Log @LoggingSettings -Comment "Failed to back up network: $NetworkName. Ensure the network name is correct." -Level ERROR
                return
            }
            Write-Log @LoggingSettings -Comment "Backup completed successfully for network: $NetworkName" -Level INFO
        } else {
            $result = netsh wlan export profile key=clear folder="$BackupPath" 2>&1
            if ($result -notmatch "successfully exported") {
                Write-Log @LoggingSettings -Comment "Failed to back up one or more networks. Check permissions and network availability." -Level ERROR
                return
            }
            Write-Log @LoggingSettings -Comment "Backup complete for all network profiles." -Level INFO
        }
    } catch {
        Write-Log @LoggingSettings -Comment "Error during backup operation: $_" -Level ERROR
    }
}
...
function Restore {
    ...
    if (-not (Test-Path -Path $BackupPath)) {
        Write-Log @LoggingSettings -Comment "Backup folder not found. Cannot restore profiles. Please ensure backups are available." -Level ERROR
        return
    }

    Write-Log @LoggingSettings -Comment "Restoring Wi-Fi networks..." -Level INFO
    try {
        if ($NetworkName) {
            Write-Log @LoggingSettings -Comment "Restoring Wi-Fi network: $NetworkName..." -Level INFO
            ...
            if ($File) {
                ...
                if ($result -notmatch "added to the system successfully") {
                    Write-Log @LoggingSettings -Comment "Failed to restore network: $NetworkName. Ensure the backup exists and is valid." -Level ERROR
                    return
                }
                Write-Log @LoggingSettings -Comment "Restored network: $NetworkName" -Level INFO
            } else {
                Write-Log @LoggingSettings -Comment "No backup found for network: $NetworkName" -Level ERROR
            }
        } else {
            ...
            foreach ($File in $Files) {
                $result = netsh wlan add profile filename="$($File.FullName)" user=current 2>&1
                if ($result -notmatch "added to the system successfully") {
                    Write-Log @LoggingSettings -Comment "Failed to restore profile from file: $($File.FullName)" -Level ERROR
                } else {
                    Write-Log @LoggingSettings -Comment "Restored profile from file: $($File.FullName)" -Level INFO
                }
            }
        }
    } catch {
        Write-Log @LoggingSettings -Comment "Error during restore operation: $_" -Level ERROR
    }
}
...
function Wipe {
    ...
    Write-Log @LoggingSettings -Comment "WARNING: This will delete ALL saved Wi-Fi network profiles on this system. This action is irreversible and cannot be undone unless you have a backup." -Level WARNING
    ...
    if (-not $Confirm) {
        Write-Log @LoggingSettings -Comment "Wipe operation cancelled by user." -Level WARNING
        return
    }

    Write-Log @LoggingSettings -Comment "Deleting all saved Wi-Fi network profiles..." -Level INFO

    try {
        $result = netsh wlan delete profile name="*" 2>&1
        if ($result -match "Profile.*on interface.*successfully deleted") {
            Write-Log @LoggingSettings -Comment "Successfully deleted all saved Wi-Fi profiles." -Level INFO
        } elseif ($result -match "No profiles.*") {
            Write-Log @LoggingSettings -Comment "No Wi-Fi profiles were found to delete." -Level WARNING
        } else {
            Write-Log @LoggingSettings -Comment "Failed to delete Wi-Fi profiles. Error: $result" -Level ERROR
        }
    } catch {
        Write-Log @LoggingSettings -Comment "Error during Wipe operation: $_" -Level ERROR
    }
}
...
function ExportToPlainText {
    ...
    Write-Log @LoggingSettings -Comment "WARNING: This will export Wi-Fi network names and passwords to a plain text file. Ensure this file is kept secure." -Level WARNING
    ...
    if (-not $Confirm) {
        Write-Log @LoggingSettings -Comment "Export operation cancelled by user." -Level WARNING
        return
    }
    ...
    Write-Log @LoggingSettings -Comment "Exporting Wi-Fi profiles and passwords to $ExportPath..." -Level INFO
        ...
        if ($Profiles) {
            ...
            foreach ($Profile in $Profiles) {
                try {
					...
					Write-Log @LoggingSettings -Comment "Network Name: $Profile, Password: <hidden>" -Level INFO
					
                    $Output += "Network Name: $Profile`nPassword: $Password`n`n"
                } catch {
                    Write-Log @LoggingSettings -Comment "Error retrieving details for profile: $Profile. Skipping." -Level WARNING
                    continue
                }
            }

            $Output | Set-Content -Path $ExportPath
            Write-Log @LoggingSettings -Comment "Export complete: $ExportPath." -Level INFO
        } else {
            Write-Log @LoggingSettings -Comment "No Wi-Fi profiles found to export." -Level INFO
        }
    } catch {
        Write-Log @LoggingSettings -Comment "Error during Export operation: $_" -Level ERROR
    }
}
...
function CompressBackup {
    ...
    Write-Log @LoggingSettings -Comment "Compressing backup folder..." -Level INFO

    try {
        if (-not (Test-Path $BackupPath)) {
            Write-Log @LoggingSettings -Comment "Backup path does not exist. Perform a backup first." -Level ERROR
            return
        }
		...
		if (Test-Path $ZipFilePath) {
			Write-Log @LoggingSettings -Comment "A ZIP file with the same name already exists: $ZipFilePath." -Level WARNING
			...
			if (-not $ConfirmOverwrite) {
				Write-Log @LoggingSettings -Comment "Compression operation cancelled by user." -Level WARNING
				return
			}
		}
        ...
        if (-not (Test-Path $ZipFilePath)) {
            Write-Log @LoggingSettings -Comment "Error: ZIP file was not created successfully." -Level ERROR
        } else {
            Write-Log @LoggingSettings -Comment "Backup folder compressed: $ZipFilePath" -Level INFO
        }
    } catch {
        Write-Log @LoggingSettings -Comment "Error compressing backup folder: $_" -Level ERROR
    }
}
...
function Cleanup {
	...
	# Checks if the backup folder exists, if it doesn't it reports that the backup folder doesn't exist and exits as there is no work to do
	if (-not (Test-Path -Path $BackupPath)) {
    Write-Log @LoggingSettings -Comment "Backup folder not found. Nothing to clean up." -Level INFO
    return
	}

    Write-Log @LoggingSettings -Comment "WARNING: This will delete the backup folder and all saved Wi-Fi profile backups." -Level WARNING
    Write-Log @LoggingSettings -Comment "WARNING: This action is irreversible. Ensure you no longer need these files before proceeding." -Level WARNING

    $Confirm = if ($UseGUI) { Confirm-Action "Delete backup folder?" } else { Read-InYesNo -Prompt "Delete backup folder?" }
    if (-not $Confirm) {
        Write-Log @LoggingSettings -Comment "Cleanup operation cancelled by user." -Level WARNING
        return
    }

    Write-Log @LoggingSettings -Comment "Deleting backup folder..." -Level INFO
    Remove-Item -Recurse -Force $BackupPath
    Write-Log @LoggingSettings -Comment "Done: Backup folder has been deleted." -Level INFO
}
...
# Function to update the ListView with networks
Function Update-NetworkListView {
	param ([array]$Items)

	$NetworkListView.Items.Clear()  # Clear previous entries
	foreach ($Item in $Items) {
		$ListItem = New-Object System.Windows.Forms.ListViewItem($Item)
		$NetworkListView.Items.Add($ListItem)
	}
}
...
function Show-GUI {
	...
	if (-not (Get-Command -Name Add-Type -ErrorAction SilentlyContinue)) {
		Write-Log @LoggingSettings -Comment "Windows Forms components are not available. Falling back to CLI mode." -Level WARNING
		$UseGUI = $false		
		return
	}
    ...
    $Form.Size = New-Object System.Drawing.Size(750, 700)  # Size of the form window
    ...
    $NetworkLabel.Location = New-Object System.Drawing.Point(10, 50)  # Position the label beside the TextBox
    ...
    $NetworkTextBox.Location = New-Object System.Drawing.Point(250, 45)  # Adjusted position of the TextBox
	...
	# Add a ListView for saved networks
	$NetworkListView = New-Object System.Windows.Forms.ListView
	$NetworkListView.View = 'Details'  # Display as a detailed list
	$NetworkListView.FullRowSelect = $true
	$NetworkListView.GridLines = $true
	$NetworkListView.MultiSelect = $false
	$NetworkListView.Scrollable = $true
	$NetworkListView.Size = New-Object System.Drawing.Size(680, 150)
	$NetworkListView.Location = New-Object System.Drawing.Point(10, 90) # Positioned below the network name TextBox
	$NetworkListView.Columns.Add("Network Name", -2, [System.Windows.Forms.HorizontalAlignment]::Left)
	$Form.Controls.Add($NetworkListView)
    ...
    $LogBox.Size = New-Object System.Drawing.Size(680, 200)  # Set the size of the log box
    $LogBox.Location = New-Object System.Drawing.Point(10, 500)  # Adjust position to create space for buttons
    ...
    $ButtonMargin = 20  # Space between buttons
    $ButtonStartY = 300  # Start position for the buttons (below the ListView)

    # Add buttons with actions
    $ButtonActions = @(
        @{Text = "Backup Profiles"; Action = { Backup -NetworkName $NetworkTextBox.Text }},
        @{Text = "Restore Profiles"; Action = { Restore -NetworkName $NetworkTextBox.Text }},
        @{Text = "Wipe System Profiles"; Action = { Wipe }},
        @{Text = "Export To Plain-Text"; Action = { ExportToPlainText }},
        @{Text = "Cleanup Backups"; Action = { Cleanup }},
        @{Text = "List Current Profiles"; Action = { ListSavedNetworks }},
        @{Text = "List Backup Profiles"; Action = { ListBackedUpNetworks }},
        @{Text = "Compress Backup to ZIP"; Action = { CompressBackup }}
    )

    for ($i = 0; $i -lt $ButtonActions.Count; $i++) {
        $X = 10 + (($ButtonWidth + $ButtonMargin) * ($i % 4))  # Calculate X position
        $Y = $ButtonStartY + [math]::Floor($i / 4) * ($ButtonHeight + $ButtonMargin)  # Calculate Y position
        $ButtonData = $ButtonActions[$i]

        $Form.Controls.Add((Create-StyledButton -Text $ButtonData.Text -X $X -Y $Y -OnClick $ButtonData.Action))
    }
...
<#
    ...
    $Form.Controls.Add((Create-StyledButton -Text "Backup Profiles" -X $Margin -Y ($Margin + 120) -OnClick {
        ...
        Write-Log @LoggingSettings -Comment "Initiating Backup for network: $NetworkName via GUI..." -Level "Info"
    ...
    $Form.Controls.Add((Create-StyledButton -Text "Restore Profiles" -X (($Margin * 2) + $ButtonWidth) -Y ($Margin + 120) -OnClick {
        $NetworkName = $NetworkTextBox.Text  # Get the network name from the TextBox
        Write-Log @LoggingSettings -Comment "Initiating Restore for network: $NetworkName via GUI..." -Level "Info"
    ...
    $Form.Controls.Add((Create-StyledButton -Text "Wipe System Profiles" -X (($Margin * 3) + ($ButtonWidth * 2)) -Y ($Margin + 120) -OnClick {
        Write-Log @LoggingSettings -Comment "Initiating Wipe via GUI..." -Level "Info"  # Log the action
    ...
    $Form.Controls.Add((Create-StyledButton -Text "Export To Plain-Text" -X (($Margin * 4) + ($ButtonWidth * 3)) -Y ($Margin + 120) -OnClick {
        Write-Log @LoggingSettings -Comment "Initiating Export via GUI..." -Level "Info"  # Log the action
    ...
    $Form.Controls.Add((Create-StyledButton -Text "Cleanup Backups" -X $Margin -Y (($Margin * 2) + $ButtonHeight + 120) -OnClick {
        Write-Log @LoggingSettings -Comment "Initiating Cleanup via GUI..." -Level "Info"  # Log the action
        Cleanup  # Call the Cleanup function
    }))

    $Form.Controls.Add((Create-StyledButton -Text "List Current System Profiles" -X $Margin -Y (($Margin * 2) + $ButtonHeight + 120) -OnClick {
        Write-Log @LoggingSettings -Comment "Listing current Wi-Fi networks via GUI..." -Level "Info"  # Log the action
        ListSavedNetworks  # Call the ListSavedNetworks function
    }))

    $Form.Controls.Add((Create-StyledButton -Text "List Backup Profiles" -X (($Margin * 2) + $ButtonWidth) -Y (($Margin * 2) + $ButtonHeight + 120) -OnClick {
        Write-Log @LoggingSettings -Comment "Listing backed-up Wi-Fi networks via GUI..." -Level "Info"  # Log the action
    ...
    $Form.Controls.Add((Create-StyledButton -Text "Compress Backup to ZIP" -X (($Margin * 4) + ($ButtonWidth * 3)) -Y (($Margin * 2) + $ButtonHeight + 120) -OnClick {
        Write-Log @LoggingSettings -Comment "Initiating Compression via GUI..." -Level "Info"  # Log the action
...
Write-Log @LoggingSettings -Comment "Script execution complete." -Level INFO
